// import {generateTokenURI} from 'loot-for-everyone-common';
...
export class RandomTokenStore extends BaseStore<NFTs> {
  ...
  generate(num: number): void {
    ...
    try {
      const fromStorage = localStorage.getItem('_loot_generated');
      if (fromStorage) {
        try {
    ...
    if (!data) {
      ...
      try {
        localStorage.setItem('_loot_generated', JSON.stringify(data));
      } catch (e) {
        console.error(e);
      }
    }
